...
def think(dt):
    ...
    if enemy:
        dx = enemy["x"] - self_x
        dz = enemy["z"] - self_z
        if self_hp < self_max_hp * 0.2:
            move(-dx + ax, -dz + az)
        else:
            move(dx + ax, dz + az)
        if enemy["distance"] < 8.0 and _fire_cd <= 0.0:
            fire(dx, dz)
            _fire_cd = 0.15
    else:
